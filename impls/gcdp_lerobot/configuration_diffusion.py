...
class DiffusionConfig(PreTrainedConfig):
    ...
    Args:
        ...
        goal_conditioning: Append an episode-final image goal to the visual conditioning (GC-DP).
            Training images contain n_obs_steps history frames followed by one goal frame.
            select_action expects each camera as (B, 2, C, H, W): current image, then goal image.
    ...
    def __post_init__(self):
        ...
        # The final encoder stage is an identity, so only preceding stages downsample.
        downsampling_factor = 2 ** (len(self.down_dims) - 1)
        if self.horizon % downsampling_factor != 0:
            raise ValueError(
                "The horizon should be an integer multiple of the downsampling factor (which is determined "
                f"by `len(down_dims)`). Got {self.horizon=} and {self.down_dims=}"
            )
        max_action_steps = self.horizon if self.goal_conditioning else self.horizon - self.n_obs_steps + 1
        if self.n_action_steps > max_action_steps:
            raise ValueError(f"`n_action_steps` must be <= {max_action_steps} for this action alignment.")
    ...
    def validate_features(self) -> None:
        if self.goal_conditioning and not self.image_features:
            raise ValueError("Image goal conditioning requires at least one image input.")
